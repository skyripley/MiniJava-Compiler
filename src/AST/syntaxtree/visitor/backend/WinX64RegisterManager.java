...
import AST.syntaxtree.visitor.regalloc.GraphColorAllocator;
import AST.syntaxtree.visitor.regalloc.RegisterAllocator;
...
public class WinX64RegisterManager implements RegisterManager
{
	...
	private final Register[] callParamRegisters = { new Register("rcx"),
			new Register("rdi"), new Register("r8"), new Register("r9") };
	private final Register[] calleeSaveRegisters = { new Register("rbx"), new Register("r12"),
			...
			new Register("rdx"), new Register("rsi") };
	...
	{
		Map<String, AST.syntaxtree.visitor.regalloc.Value> callerSavedAllocation = allocator
				.allocateRegisters(f, k);
		idToValueMapping.clear();
		for (Entry<String, AST.syntaxtree.visitor.regalloc.Value> entry : callerSavedAllocation
				.entrySet())
		{
			if (entry.getValue() instanceof AST.syntaxtree.visitor.regalloc.Register)
				idToValueMapping.put(
						entry.getKey(),
						new Register(
								getRegisterByIndex(((AST.syntaxtree.visitor.regalloc.Register) entry
										.getValue()).getRegisterIndex())));
			else if (entry.getValue() instanceof AST.syntaxtree.visitor.regalloc.StackOffset)
				idToValueMapping.put(
						entry.getKey(),
						getStackOffset(((AST.syntaxtree.visitor.regalloc.StackOffset) entry.getValue())
								.getStackOffset() * -1 - 1));
